---
layout: default
title: "Resources"
---

<section class="references-index">
  <h2>References</h2>

  <p class="resources-intro">
    Supplementary materials that provide context for the posters and articles in this zine.
    Explore archival poster collections and curated online repositories for deeper research.
  </p>

  <ul class="resources-list">
    <li class="resources-list-item">
      <a href="{{ '/resources/historical-posters/' | relative_url }}" class="resource-title">
        Historical Posters
      </a>
      <span class="resource-subtitle-inline">
        Archival posters from 1960s–1970s protest movements
      </span>
    </li>

    <li class="resources-list-item">
      <a href="{{ '/resources/online-repositories/' | relative_url }}" class="resource-title">
        Online Repositories
      </a>
      <span class="resource-subtitle-inline">
        Digital archives & research databases for further exploration
      </span>
    </li>
  </ul>
</section>
